...
int main(void) {
    ...
    sd_command(17, 0, 0x01, r1, 1, 1); // READ_SINGLE_BLOCK, card responds with 0x00 byte (accepted)
    ...
    for (uint16_t i = 0; i < 512; i++) {
        data_block[i] = spi_transfer(0xFF); 
    }

    #if DEBUG
        usart_send_string("Signature: ");
        usart_send_num(data_block[510]);
        usart_send_char(' ');
        usart_send_num(data_block[511]);
        usart_send_string("\n\r");
    #endif
